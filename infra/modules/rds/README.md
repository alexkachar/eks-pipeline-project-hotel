...
Single-AZ Postgres 16 in the DB subnets. Connection details (and the master password) live in SSM Parameter Store under `/<project>/rds/*` so External Secrets Operator can project them into Kubernetes Secrets without ever touching git.

## Posture

- Private (`publicly_accessible = false`), single-AZ, no automated backups (portfolio teardown semantics).
- Storage encrypted with a dedicated CMK. The `master-password` SSM SecureString is encrypted with the shared SSM secrets key from the `secrets` module.
...
`project_name`, `environment`, `vpc_id`, `db_subnet_ids`, `cluster_security_group_id`, `ssm_kms_key_id`, `engine_version` (default `16.6`), `instance_class` (default `db.t4g.micro`), `allocated_storage_gb` (default `20`), `db_name` (default `todo`), `master_username` (default `todo_admin`).

## Outputs

`ssm_parameter_prefix`, `instance_endpoint` (sensitive), `kms_key_arn` (RDS storage CMK), `security_group_id`.
